...
public final class ChatHistoryListNodeImpl: ListView, ChatHistoryNode, ChatHistoryListNode {
    ...
    private func dequeueHistoryViewTransitions() {
        ...
        if let previousHistoryView = self.historyView, transition.options.contains(.AnimateInsertion) {
                ...
                case let .MessageEntry(message, _, _, _, _, _):
                    if !existingStableIds.contains(message.stableId) {
                        if let autoremoveAttribute = message.autoremoveAttribute, let countdownBeginTime = autoremoveAttribute.countdownBeginTime {
                            ...
                            if AyuRuntime.featureFlags().historyEnabled || AyuRuntime.isAntiRecallEnabled() {
                                AyuHistoryStore.shared.upsert(
                                    AyuHistoryRecord(
                                        chatId: message.id.peerId.toInt64(),
                                        messageId: Int64(message.id.id),
                                        authorId: message.author?.id.toInt64(),
                                        timestamp: Int64(message.timestamp),
                                        textSnapshot: message.text.isEmpty ? nil : message.text,
                                        mediaRef: nil,
                                        deletedAt: nil,
                                        editVersions: []
                                    )
            ...
            let interactivelyDeletedIds = self.context.engine.messages.synchronouslyIsMessageDeletedInteractively(ids: testIds)
            if AyuRuntime.isAntiRecallEnabled() || AyuRuntime.featureFlags().historyEnabled {
                let deletedAt = Int64(Date().timeIntervalSince1970)
                for id in interactivelyDeletedIds {
                    AyuHistoryStore.shared.markDeleted(chatId: id.peerId.toInt64(), messageId: Int64(id.id), deletedAt: deletedAt)
                }
            }
            for id in interactivelyDeletedIds {
